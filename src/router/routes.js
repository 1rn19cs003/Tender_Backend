...
module.exports = function (app, db) {
    ...
    async function (req, res, next) {
      ...
      if (
        ...
        db.collection("files").findOne(
          ...
          { projection: { _id: 1, tenderName: 1, profile: 1 } },
          (error, result) => {
  ...
  app.post("/upload_edm", upload_EDM, async function (req, res, next) {
    ...
    // check if any value is not null
    else if (size != 0 && f && k.tenderName && k.email) {
      // check if record already exists...
      db.collection("tender_files").findOne(
        {
          email: k.email,
    ...
    db.collection("files").findOne(
      ...
      (error, result) => {
        if (result && result._id) {
          ...
          const resu = db
            .collection("files")
            .deleteOne({ tenderName: k.tenderName });
          res.send(resu);
        } else {
          // console.log(results);
          // res.json(results);
      ...
      (error, result) => {
        if (result && result._id) {
          ...
          const resu = db
            ...
            .deleteMany({ tenderName: k.tenderName });
          // res.send(resu);
          console.log(resu);
          res.send(resu);
        } else {
    ...
    db.collection("tender_files").findOne(
      ...
      (error, result) => {
        if (result && result._id) {
          ...
          const resu = db
            .collection("tender_files")
            .deleteMany({ tenderName: k.tenderName, email: k.email });
          // res.send(resu);
          console.log(resu);
          res.send(resu);
        } else {
